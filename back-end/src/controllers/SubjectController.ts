...
async function getSubjectByName(
  name: string,
  excludeId?: number,
  res?: Response,
): Promise<Subject | null | Response> {
  if (!name || name.trim() === '')
...
export class SubjectController {
  ...
  static async create(req: Request<{}, {}, { name: string }>, res: Response) {
    try {
      const { name } = req.body;
      const check = await getSubjectByName(name, undefined, res);
      if ((check as Response).headersSent) return;
      const repo = AppDataSource.getRepository(Subject);
  ...
  static async update(
    ...
    try {
      ...
      const check = await getSubjectByName(name, id, res);
      if ((check as Response).headersSent) return;
